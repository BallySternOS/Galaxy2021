...
#define MACHINE_STATE_ADJUST_SFX_AND_SOUNDTRACK -19
#define MACHINE_STATE_ADJUST_MUSIC_VOLUME       -20
#define MACHINE_STATE_ADJUST_SFX_VOLUME         -21
#define MACHINE_STATE_ADJUST_CALLOUTS_VOLUME    -22
#define MACHINE_STATE_ADJUST_TOURNAMENT_SCORING -23
#define MACHINE_STATE_ADJUST_TILT_WARNING       -24
#define MACHINE_STATE_ADJUST_AWARD_OVERRIDE     -25
#define MACHINE_STATE_ADJUST_BALLS_OVERRIDE     -26
...
#define MACHINE_STATE_ADJUST_DIM_LEVEL          -30
...
void SetDropTargets(byte newTargetStatus) {
  ...
  if (~newTargetStatus & DropTargetStatus) {
    BSOS_PushToSolenoidStack(SOL_DROP_TARGET_RESET, 12);
    nextDropTime += 125;
  }
  ...
  for (int count=0; count<4; count++) {
    if (newTargetStatus & (1<<count)) {
      ...
      BSOS_PushToTimedSolenoidStack(targetToDrop, 8, nextDropTime);
      nextDropTime += 100;
    }
  }
...
byte GetDropTargetStatus() {
  byte returnSwitches = 0;
  returnSwitches |= (BSOS_ReadSingleSwitchState(SW_RED_DROP_TARGET));
  returnSwitches |= (BSOS_ReadSingleSwitchState(SW_BLUE_DROP_TARGET)<<1);
  returnSwitches |= (BSOS_ReadSingleSwitchState(SW_YELLOW_DROP_TARGET)<<2);
  returnSwitches |= (BSOS_ReadSingleSwitchState(SW_BLACK_DROP_TARGET)<<3);
...
void SpotNextDropTarget() {
  ...
  if (solToDrop!=0) {
    BSOS_PushToSolenoidStack(solToDrop, 3);
  }
...
void ShowBonusLamps() {
  if ((GameMode & GAME_BASE_MODE) == GAME_MODE_WIZARD_QUALIFIED) {
    for (int count=0; count<8; count++) BSOS_SetLampState(LAMP_BONUS_40+count, 0);
  } else if ((GameMode & GAME_BASE_MODE) == GAME_MODE_SKILL_SHOT) {
    byte bonusPhase = ((CurrentTime-GameModeStartTime)/100)%12;    
    for (int count=0; count<8; count++) {
      if ((LAMP_BONUS_40+count)!=LAMP_BONUS_10) BSOS_SetLampState(LAMP_BONUS_40+count, (BonusLampScanAnimation[bonusPhase])==count);
      else BSOS_SetLampState(LAMP_BONUS_10, 1);
    }
  } else {
    if (Bonus[CurrentPlayer]>1) {
      ...
      for (int count=0; count<8; count++) {
        if (curBonus>BonusLampValues[count]) {
          BSOS_SetLampState(LAMP_BONUS_40+count, 1);
          curBonus -= BonusLampValues[count];
        } else {
          BSOS_SetLampState(LAMP_BONUS_40+count, 0);
        }
      }
    } else {
      for (int count=0; count<7; count++) BSOS_SetLampState(LAMP_BONUS_40+count, 0);
      BSOS_SetLampState(LAMP_BONUS_2, 1, 0, 200);
    }
  }
}
...
void ShowSaucerLamps() {
  if ((GameMode & GAME_BASE_MODE) == GAME_MODE_SKILL_SHOT) {
    ...
    for (int count=0; count<5; count++) {
      BSOS_SetLampState(LAMP_SAUCER_2K-count, (count==lampNum));
    }
  } else if (SideQuestQualifiedEndTime!=0 || ((GameMode & GAME_BASE_MODE)==GAME_MODE_WIZARD_QUALIFIED)) { 
    byte lampPhase = (CurrentTime/100)%6;
    for (int count=0; count<5; count++) {
      BSOS_SetLampState(LAMP_SAUCER_2K-count, (count==lampPhase)||count==(lampPhase-1));
    }
  } else {
    if (SaucerHitAnimationEnd) {
      for (int count=0; count<5; count++) {
        BSOS_SetLampState(LAMP_SAUCER_2K-count, (count==SaucerHitLampNum), 0, 120);
      }
      ...
      for (int count=0; count<5; count++) {
        BSOS_SetLampState(LAMP_SAUCER_2K-count, (count+1)==(CurrentSaucerValue/2), 0, flashing);
      }
    }
  }
}
...
void ShowBonusXLamps() {
  if ((GameMode & GAME_BASE_MODE) == GAME_MODE_SKILL_SHOT || (GameMode & GAME_BASE_MODE) == GAME_MODE_WIZARD_QUALIFIED) {
    for (int count=0; count<4; count++) BSOS_SetLampState(LAMP_BONUS_5X+count, 0);
  } else {
    if (BonusX[CurrentPlayer]==1) {
      for (int count=0; count<4; count++) BSOS_SetLampState(LAMP_BONUS_5X+count, 0);
    } else if (BonusX[CurrentPlayer]<6) {
      for (int count=0; count<4; count++) BSOS_SetLampState(LAMP_BONUS_5X+count, BonusX[CurrentPlayer]==(5-count));
    } else {
      BSOS_SetLampState(LAMP_BONUS_5X, BonusX[CurrentPlayer]>6, 0, BonusX[CurrentPlayer]>9?200:0);
      BSOS_SetLampState(LAMP_BONUS_4X, BonusX[CurrentPlayer]==6 || BonusX[CurrentPlayer]==9);
      BSOS_SetLampState(LAMP_BONUS_3X, BonusX[CurrentPlayer]==8);
      BSOS_SetLampState(LAMP_BONUS_2X, BonusX[CurrentPlayer]<8);
    }
  }
}
...
void ShowSpinnerLamp() {
  if ((GameMode & GAME_BASE_MODE) == GAME_MODE_WIZARD_QUALIFIED) {
    BSOS_SetLampState(LAMP_SPINNER_1000, 0);
  } else if ((GameMode & GAME_BASE_MODE) == GAME_MODE_MISSION_FUELING) {
    if (CurrentFuel<FuelRequired) BSOS_SetLampState(LAMP_SPINNER_1000, 1, 0, 400);
    else BSOS_SetLampState(LAMP_SPINNER_1000, 0);
  } else if (SpinnerGoalReachedTime || (GameMode&GAME_MODE_FLAG_REFUEL)) {
    BSOS_SetLampState(LAMP_SPINNER_1000, 1, 0, 125);
...
void ShowGalaxyLamps() {
  if ((GameMode & GAME_BASE_MODE) == GAME_MODE_WIZARD_QUALIFIED) {
    ...
    for (int count=0; count<6; count++) {
      BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, count==(5-lampPhase));
      BSOS_SetLampState(LAMP_TOPLANE_G-count, 0);
    }
    ...
    for (int count=0; count<6; count++) {
      BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, count==lampPhase);
      BSOS_SetLampState(LAMP_TOPLANE_G-count, 0);
    }
    if (CurrentTime>(GalaxyBounceAnimationStart+660)) GalaxyBounceAnimationStart = 0;
  } else if ((GameMode & GAME_BASE_MODE) == GAME_MODE_SKILL_SHOT) {
    for (int count=0; count<6; count++) {
      BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, 0);
      BSOS_SetLampState(LAMP_TOPLANE_G-count, 0);
    }
    ...
    for (int count=0; count<6; count++) {
      ...
      if (GalaxyLetterAnimationEnd[count]) {        
        BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, 1, 1, 150);
      } else {
        BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, GalaxyTurnaroundLights[CurrentPlayer]&lampBit);
      }
      
      BSOS_SetLampState(LAMP_TOPLANE_G-count, (GalaxyStatus[CurrentPlayer]&lampBit)==0);
      lampBit *= 2;  
    }
    ...
    for (int count=0; count<6; count++) {
      BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, GalaxyTurnaroundLights[CurrentPlayer]&lampBit);
      BSOS_SetLampState(LAMP_TOPLANE_G-count, 1, 0, 125);
      lampBit *= 2;
    }
    ...
    for (int count=0; count<6; count++) {
      if (GalaxyLetterAnimationEnd[count]) {
        if (CurrentTime>GalaxyLetterAnimationEnd[count]) {
          ...
          if (!ShowGalaxyTurnaroundDraw) BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, lampPhase);
          BSOS_SetLampState(LAMP_TOPLANE_G-count, (lampPhase==0));
        }
      } else {
        if (!ShowGalaxyTurnaroundDraw) BSOS_SetLampState(LAMP_GALAXY_TURNAROUND_G-count, GalaxyTurnaroundLights[CurrentPlayer]&lampBit);
        BSOS_SetLampState(LAMP_TOPLANE_G-count, (GalaxyStatus[CurrentPlayer]&lampBit)==0);
      }
      lampBit *= 2;  
...
void ShowMissionLamps() {
  ...
  if ((GameMode & GAME_BASE_MODE) == GAME_MODE_SKILL_SHOT || (GameMode & GAME_BASE_MODE) == GAME_MODE_WIZARD_QUALIFIED) {
    for (int count=0; count<MISSION_TO_SUN; count++) {
      BSOS_SetLampState(LAMP_MERCURY-count, 0);
    }
    BSOS_SetLampState(LAMP_SUN_SPECIAL, (GameMode & GAME_BASE_MODE) == GAME_MODE_WIZARD_QUALIFIED, 0, 125);
    ...
    for (byte count=0; count<8; count++) {
      BSOS_SetLampState(LAMP_MERCURY-count, (count==dtLampPhase || count==suLampPhase));
    }
    ...
    if ((GameMode & GAME_BASE_MODE)!=GAME_MODE_UNSTRUCTURED_PLAY) {
      ...
      BSOS_SetLampState(LAMP_MERCURY-NextMission, 1, 0, missionFlash);
    }
    BSOS_SetLampState(LAMP_SUN_SPECIAL, 0);
  }
...
void ShowLaneLamps() {
  if ((GameMode & GAME_BASE_MODE) == GAME_MODE_SKILL_SHOT || (GameMode & GAME_BASE_MODE) == GAME_MODE_WIZARD_QUALIFIED) {
    BSOS_SetLampState(LAMP_LEFT_OUTLANE, 0);
    BSOS_SetLampState(LAMP_RIGHT_OUTLANE, 0);
    BSOS_SetLampState(LAMP_LEFT_EXTRABALL, 0);
    BSOS_SetLampState(LAMP_RIGHT_EXTRABALL, 0);
    BSOS_SetLampState(LAMP_LEFT_SPECIAL, 0);
    BSOS_SetLampState(LAMP_RIGHT_SPECIAL, 0);
    BSOS_SetLampState(LAMP_SPOT_TARGET_INLANES, 0);
    BSOS_SetLampState(LAMP_10X_SCORE_WHEN_LIT, 1, 0, 100);
  } else {
    BSOS_SetLampState(LAMP_LEFT_OUTLANE, OutlaneStatus==1);
    BSOS_SetLampState(LAMP_RIGHT_OUTLANE, OutlaneStatus==2);
    BSOS_SetLampState(LAMP_LEFT_EXTRABALL, ExtraBallLit[CurrentPlayer]==1);
    BSOS_SetLampState(LAMP_RIGHT_EXTRABALL, ExtraBallLit[CurrentPlayer]==2);
    BSOS_SetLampState(LAMP_LEFT_SPECIAL, SpecialLit[CurrentPlayer]==1);
    BSOS_SetLampState(LAMP_RIGHT_SPECIAL, SpecialLit[CurrentPlayer]==2);
    BSOS_SetLampState(LAMP_SPOT_TARGET_INLANES, MidlaneStatus, 0, (MidlaneStatus==1)?250:0);
    BSOS_SetLampState(LAMP_10X_SCORE_WHEN_LIT, 0);
  }
...
void AddCoinToAudit(byte switchHit) {
  ...
  unsigned int coinAuditStartByte = 0;
  ...
  for (int count=0; count<SOUND_QUEUE_SIZE; count++) {
    if (SoundQueue[count].playTime!=0 && SoundQueue[count].playTime<pullTime) {
      if (SoundQueue[count].soundFunction==SB300_SOUND_FUNCTION_SQUARE_WAVE) {
        BSOS_PlaySB300SquareWave(SoundQueue[count].soundRegister, SoundQueue[count].soundByte);   
      } else if (SoundQueue[count].soundFunction==SB300_SOUND_FUNCTION_ANALOG) {
        BSOS_PlaySB300Analog(SoundQueue[count].soundRegister, SoundQueue[count].soundByte);   
      }
      SoundQueue[count].playTime = 0;
...
int InitNewBall(bool curStateChanged, byte playerNum, int ballNum) {
  ...
  if (curStateChanged) {
    ...
    BSOS_PushToTimedSolenoidStack(SOL_DROP_TARGET_RESET, 12, CurrentTime + 200);
...
//          ShowLampAnimation(0, 80, CurrentTime, 14, false, false);
    ...
    case GAME_MODE_WIZARD_START:
      ...
      if (CurrentTime<(GameModeStartTime+3700)) {
        ...
        if (lampPhase==0) BSOS_TurnOffAllLamps();
...
void HandleDropTargetHit(byte switchHit) {
  ...
  if (currentStatus==0x0F) {
    if (ResetDropTargetStatusTime==0 && ((GameMode&GAME_BASE_MODE)!=GAME_MODE_WIZARD)) {
      AddToBonus(2);
      BSOS_PushToTimedSolenoidStack(SOL_DROP_TARGET_RESET, 12, CurrentTime + 750);
      ...
      switch (switchHit) {
        ...
        case SW_BLACK_STANDUP:
          ...
          if ((DropTargetStatus&0x08)==0) {
            ...
            BSOS_PushToSolenoidStack(SOL_BLACK_DROP_TARGET, 3);
          }
        ...
        case SW_YELLOW_STANDUP:
          ...
          if ((DropTargetStatus&0x04)==0) {
            // drop the Black target
            BSOS_PushToSolenoidStack(SOL_YELLOW_DROP_TARGET, 3);
          } else {
            if (DEBUG_MESSAGES) {
              char buf[256];
        ...
        case SW_BLUE_STANDUP:
          ...
          if ((DropTargetStatus&0x02)==0) {
            // drop the Black target
            BSOS_PushToSolenoidStack(SOL_BLUE_DROP_TARGET, 3);
          }
          if ( (GameMode & GAME_MODE_FLAG_PIRATE_ENCOUNTER) ) {
            CurrentScores[CurrentPlayer] += ScoreMultiplier * AWARD_FOR_PIRATE_ENCOUNTER;
            PlaySoundEffect(SOUND_EFFECT_PIRATE_COLLECT, ConvertVolumeSettingToGain(SoundEffectsVolume));
